...
GitTeleclone_ssh.prototype.connect = function(args) {
    ...
    if( !platform.match(/^win/) ) {
        if( 1 || allow_ssh_agent ) {
            ...
            ssh_args['privateKey'] = __getUserHome() + '/.ssh/id_rsa';
        shell.cat(ssh_args['privateKey']);
    }
    }
    else {
            ssh_args['privateKey'] = fs.readFileSync('.ssh/id_rsa');
        //~ shell.cat(ssh_args['privateKey']);
    }
    ...
    ssh.on('ready', function() {
        ...
        _this._run_remote('ls ' + _this.target.pathname + ' ; echo $?', function(ret) {
            if( !ret.output.match(/0$/m) ) {
                console.log('Connected to ' + _this.target_name + ' but ' +  _this.target.pathname + ' doesn\'t appear to exist');
                process.exit();
            }
