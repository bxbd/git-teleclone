...
function TelecloneTarget_sftp(owner, target) {
    _.extend(this, { connected:  false });
    this.target = target;
    this.target_url = target.href;
    return this;
}

// TelecloneTarget_sftp = require('../teleclone-target');
    ...
    */
    ...
    ssh.on('error', function(err) {
        ...
        if( err.level == 'client-authentication' ) {
            ...
            // _this.log(err);
            if( _args && _args['callback'] ) _args['callback'].apply(this, [ err ]);
        }
    });
    ...
    ssh.on('ready', function() {
        ...
        if( _this.has_ssh() ) {
            _this._run_remote('ls ' + _this.target.pathname, function(ret) {
                ...
                _this.connected = true
                if( args && args['callback'] ) args['callback'].apply(this);
            }, true);
        }
        else {
...
TelecloneTarget_sftp.prototype._do_sftp = function(cb) {
    ...
    if( !this._sftp ) {
        this._conn.sftp(function(err, sftp) {
            if( err ) {
                _this.log('error opening sftp connection', err);
                cb(_this._sftp);
            }
            else {
                _this.log('sftp on');
